...
public class UseCaseGalileo extends UseCase {
	/**
	...
	public Constellation optimizeConstellation() {
		...
		DecisionVector decisionVectorGalileo = new DecisionVectorDemo(this.variablesList, this.inputPolygon);
		...
		ArrayList<Object> values = decisionVectorGalileo.getValues();
		...
		System.out.println("Max revisit time: " + decisionVectorGalileo.costFunction(values));

		// get and return the constellation from the decision vector
		return (decisionVectorGalileo.createConstellationFromVector(values));
